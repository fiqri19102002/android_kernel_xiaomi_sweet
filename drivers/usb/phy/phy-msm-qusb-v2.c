...
static int qusb_phy_probe(struct platform_device *pdev)
{
	...
	if (res) {
		...
		if (!IS_ERR_OR_NULL(qphy->efuse_reg)) {
			...
			qphy->need_override_tune1 = of_property_read_bool(dev->of_node, "mi,need-override_tune1");
			...
			if (size) {
				...
				if (qphy->efuse_pll_bias_seq) {
					qphy->efuse_pll_bias_seq_len = (size / sizeof(*qphy->efuse_pll_bias_seq));
			...
			if (size) {
				...
				if (qphy->override_tune1_seq) {
					qphy->override_tune1_seq_len = (size / sizeof(*qphy->override_tune1_seq));
			...
			qphy->need_override_host_tune1 = of_property_read_bool(dev->of_node, "mi,need-override_host_tune1");

			size = 0;
			of_get_property(dev->of_node, "mi,efuse-pll-bias-seq-host", &size);
			if (size) {
				qphy->efuse_pll_bias_seq_host = devm_kzalloc(dev, size, GFP_KERNEL);
				if (qphy->efuse_pll_bias_seq_host) {
					qphy->efuse_pll_bias_seq_host_len = (size / sizeof(*qphy->efuse_pll_bias_seq_host));
			...
			if (size) {
				qphy->override_tune1_seq_host = devm_kzalloc(dev, size, GFP_KERNEL);
				if (qphy->override_tune1_seq_host) {
					qphy->override_tune1_seq_host_len = (size / sizeof(*qphy->override_tune1_seq_host));
