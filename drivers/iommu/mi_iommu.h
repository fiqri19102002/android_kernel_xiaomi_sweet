...
#else
static struct rb_node *rdxtree_matched_gap(struct iova_domain *iovad, unsigned long *limit_pfn, unsigned long size, bool size_aligned)
{
	return NULL;
}

static inline int rdxtree_update_gap(struct iova_domain *iovad, struct rb_node *prev, struct rb_node *next, struct rb_node *new)
{
	return 0;
}

static inline int rdxtree_insert_gap(struct iova_domain *iovad, struct rb_node *free)
{
	return 0;
}

static inline void mi_iommu_debug_init(struct iova_domain *iovad, const char *name)
{
}
#endif
