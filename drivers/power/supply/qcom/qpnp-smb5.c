...
#ifdef CONFIG_MACH_XIAOMI_SWEET
	/* Override here */
	chg->aicl_disable = false;
	chg->dcin_uusb_over_gpio_en = false;
#else
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		...
		if (chg->six_pin_step_charge_enable) {
			...
			if (pval.intval >= CP_WARM_THRESHOLD - SOFT_JEITA_HYSTERESIS || 
				pval.intval <= CP_COOL_THRESHOLD + SOFT_JEITA_HYSTERESIS || chg->index_vfloat == MAX_STEP_ENTRIES - 1)
