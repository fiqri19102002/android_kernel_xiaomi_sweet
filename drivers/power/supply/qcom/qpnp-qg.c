...
static void process_udata_work(struct work_struct *work)
{
	...
	if ((batt_temp >= 580 || is_usb_present(chip)) && abs(last_batt_temp - batt_temp) >= 20 && is_batt_available(chip)) {
...
static int qg_get_battery_capacity(struct qpnp_qg *chip, int *soc)
{
	...
	if (chip->dt.software_optimize_ffc_qg_iterm) {
		if ((chip->fastcharge_mode_enabled) && (pre_soc == 99) && (*soc == 100) &&
			(chip->charge_status == POWER_SUPPLY_STATUS_CHARGING)) {
			rc = qg_get_battery_current(chip, &ibat);
...
static int qg_load_battery_profile(struct qpnp_qg *chip)
{
	...
	if (of_find_property(profile_node, "qcom,temp-comp-ranges", &len) &&
		...
		chip->temp_comp_cfg = kcalloc(len, sizeof(*chip->temp_comp_cfg), GFP_KERNEL);
